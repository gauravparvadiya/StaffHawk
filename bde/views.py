...
def remove_lead_generated(request):
    if request.session.has_key('username'):
        ...
        update_contract_tbl.invited_by_client = '0'
        update_contract_tbl.save()
...
def edit_contract_info(request):
    if request.session.has_key('username'):
        print(request.GET['id'])
        contract_info = Contract.objects.get(id=request.GET['id'])
        project_size = ProjectSize.objects.all()
        freelance_account = FreelanceAccount.objects.all()
        return render(request, "bde/edit_contract_info.html",
                      {'contract_info': contract_info, 'project_sizes': project_size,
...
def edit_contract_info_form_submission(request):
    if request.session.has_key('username'):
        selected_contract = Contract.objects.get(id=request.POST['contract_id'])
        if 'attachment' in request.FILES:
            print("in if")
            attachment = request.FILES['attachment']
            fs = FileSystemStorage()
        ...
        selected_contract.freelance_account_id = request.POST['freelance_account']
        ...
        if selected_contract.invited_by_client == '1' and request.POST['invited_by_client'] == '0':
            lead_generated_contract = LeadGeneratedContract.objects.get(lead_contract_id=selected_contract.id)
            lead_generated_contract.delete()
            selected_contract.lead_generated = '0'
            selected_contract.invited_by_client = request.POST['invited_by_client']
            # selected_contract.save()
        elif selected_contract.invited_by_client == '0' and request.POST['invited_by_client'] == '1':
            lead_generated_contract = LeadGeneratedContract(lead_contract_id=selected_contract.id,
                                                            bidder_account_id=bde_user.id)
